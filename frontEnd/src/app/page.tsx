"use client";
import { Calendar, ChevronDown, ChevronRight, MessageSquare, MoreHorizontal, Plus, Trash2 } from 'lucide-react';
...
const TaskManagementUI = () => {
  ...
  const updateSubtask = (parentTaskId: number, subtaskId: number, updates: Partial<Task>): void => {
    setTasks(tasks.map(task => 
      task.id === parentTaskId 
        ? { 
            ...task, 
    ...
    return (
      <>
        ...
        >
                ...
                <div className="ml-auto flex items-center">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      if (!addingSubtask) {
                        setTimeout(() => {
                          const input = document.getElementById(`new-subtask-${task.id}`)
          ...
          <td className="px-4 py-3">
            <PrioritySelector taskId={task.id} priority={task.priority} updatePriority={updateTaskPriority} />
          </td>
          <td className="px-4 py-3">
            <select
                ...
                {task.comments.length > 0 ? (
                  <div className="space-y-2">
                    {task.comments.map((comment, idx) => (
                      <div key={idx} className="p-2.5 bg-white rounded-md border shadow-sm">
                        {comment}
                      </div>
                    ))}
                  </div>
      ...
      >
        <td className="px-4 py-3">
          <div className="flex items-center justify-center">
            <input
              ...
              onChange={() => {
                updateSubtask(parentTaskId, subtask.id, { completed: !subtask.completed })
              }}
              className="rounded text-violet-500 focus:ring-violet-500"
            />
          </div>
        ...
        <td className="px-4 py-3">
          <div className="relative flex items-center">
            <input
              ...
              onChange={(e) => updateSubtask(parentTaskId, subtask.id, { dueDate: e.target.value })}
            />
            <Calendar className="w-4 h-4 text-gray-400 absolute left-2" />
          </div>
        </td>
        <td className="px-4 py-3">
          <PrioritySelector
            ...
            updatePriority={(id, priority) => updateSubtask(parentTaskId, subtask.id, { priority })}
          />
        </td>
        <td className="px-4 py-3">
          <select
            ...
            onChange={(e) => updateSubtask(parentTaskId, subtask.id, { status: e.target.value as Task["status"] })}
          >
            <option value="TO DO">To Do</option>
            <option value="IN PROGRESS">In Progress</option>
            <option value="COMPLETE">Complete</option>
    ...
    <div className="flex flex-col w-full min-h-screen bg-white text-gray-800">
      ...
      <StatusDropZone status="COMPLETE">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {tasks.filter(task => task.status === 'COMPLETE' && !task.parentId).map(task => (
      ...
      <StatusDropZone status="IN PROGRESS">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {tasks.filter(task => task.status === 'IN PROGRESS' && !task.parentId).map(task => (
              <TaskRow key={task.id} task={task} />
            ))}
      ...
      <StatusDropZone status="TO DO">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {tasks.filter(task => task.status === 'TO DO' && !task.parentId).map(task => (
              <TaskRow key={task.id} task={task} />
            ))}
