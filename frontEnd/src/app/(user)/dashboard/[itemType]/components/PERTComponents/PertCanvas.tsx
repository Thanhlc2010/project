...
const PertCanvas = forwardRef<PertCanvasRef, PertCanvasProps>(
  ({ tasks, edges: externalEdges, onEdgesChange: handleEdgesChange, onTasksChange, onInit }, ref) => {
    ...
    const createConnection = useCallback(
      ...
      [edges, nodes, setEdges, setNodes, handleEdgesChange, generateUniqueEdgeId, tasks, onTasksChange]
    );
    ...
    const findAllCriticalPaths = useCallback((): { paths: CriticalPath[]; criticalEdges: Edge[] } => {
      ...
      console.log("Critical Tasks:", criticalTasks.map((n: any) => n.id));
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        importFromJSON: (data: { nodes: Node[]; edges: Edge[]; metadata: any }) => {
          ...
          // Bước 2: Gắn thêm 2 node mặc định vào data.nodes
      ...
      [nodes, edges, setNodes, setEdges, createAllTasks, handleShowCriticalPaths, resetHighlights]
    );
    ...
    return (
      <ReactFlow
        ...
        fitView
      >
        <DroppableCanvas setNodes={setNodes} nodes={nodes} reactFlowInstance={reactFlowInstanceRef.current}>
          <Controls />
          <MiniMap />
